...
router.post("/", async (req, res) => {
    ...
    const max_users = process.env.MYSPACE_API_MAX_USERS || 1000;
...
router.post('/resetPassword', async (req, res) => {
    ...
    await ResetPassword.create({
        ...
        expire: moment.utc().add(/*config.tokenExpiry*/process.env.MYSPACE_API_TOKEN_EXPIRY, 'seconds'),
   })
    ...
    const transporter = nodemailer.createTransport({
        ...
        auth: {
            ...
            user: process.env.MYSPACE_API_EMAIL,
            pass: process.env.MYSPACE_API_MAIL_PASSWORD
        }
    });
    ...
    let mailOptions = {
        ...
        '<a href=http://' + process.env.MYSPACE_API_CLIENT_URL + 'reset/' + user.id + '/' + token + '">' + process.env.MYSPACE_API_CLIENT_URL + 'reset/' + user.id + '/' + token + '</a>' +
        '<br><br>' +
        '<p>-- Thanks</p>'
    }
